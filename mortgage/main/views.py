...
def calc_choose(request):
    ...
    return render(request, 'main/calc_choose.html', {"data": cards_data, "calcs_list_id": calcs_list_id,"calcs_count": calcs_count})

def calc_description(request, id):
    data = next((card for card in cards if id == card['id']), None)
    if not data:
        return get_object_or_404(data)
    return render(request, 'main/calc_description.html', {"data": data})

def calcs(request, id):
    calcs = next((calc for calc in calcs_list if calc['id'] == id), None)
    order_cards = []
    if calcs:
        order_cards = [card for card in cards if card['id'] in calcs['items']]
    return render(request, 'main/calcs.html',{
        'calcs':calcs_list,
